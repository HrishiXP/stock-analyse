...
export async function* generateStreamingSignal(symbol: string, news: NewsItem[]): AsyncGenerator<string, void, unknown> {
  const metadata = NSE_FO_DATA[symbol] || { sector: 'Unknown', lotSize: 0 };
  const prompt = buildSignalPrompt(symbol, metadata.sector, 'UNKNOWN', new Date().toLocaleDateString('en-US', { weekday: 'long', timeZone: 'Asia/Kolkata' }), news, metadata.lotSize);
  const model = getModel('gemini-3.5-flash');
  const streamResult = await model.generateContentStream(
    {
      contents: [
        { role: 'system', parts: [{ text: SYSTEM_PREFIX }] },
  ...
  let previousText = '';
  // idle watchdog: if no chunk emitted in 60s, attempt to terminate
  const idleMs = 60000;
  let lastAt = Date.now();
  const watchdog = setInterval(() => {
    try {
      if (Date.now() - lastAt > idleMs) {
        // attempt to stop the underlying stream if supported
        // @ts-ignore
  ...
  try {
    for await (const chunk of streamResult.stream) {
      ...
      if (currentText.length > previousText.length && currentText.startsWith(previousText)) {
        yield currentText.slice(previousText.length);
      } else {
        yield currentText;
      }
